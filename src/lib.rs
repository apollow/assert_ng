...
use syntax::ast::{mod, TokenTree, Expr};
use syntax::ptr::P;
...
fn expand_assert_ng_helper(cx: &mut ExtCtxt, expr : &P<Expr>) -> P<Expr> {
    ...
    match expr.node {
        ast::ExprBinary(oper, ref given, ref expected) => {
            let expr_left = expand_assert_ng_helper(cx, given);
            let expr_right = expand_assert_ng_helper(cx, expected);

            let if_expr = match oper {
                ast::BiEq => 
                    quote_expr!(cx, if !(*given_val == *expected_val) {
...
fn expand_assert_ng_(cx: &mut ExtCtxt, _: Span, args: &[TokenTree], debug_only: bool)
    ...
    let res = if parser.eat(&token::Comma) {
        ...
        expand_assert_ng_helper(cx, &expr)
    };
